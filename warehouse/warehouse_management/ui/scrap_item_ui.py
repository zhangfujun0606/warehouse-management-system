...
def scrap_item_window(window, warehouse1, warehouse2, update_inventory, save_data, history_log, logged_in_user):
    ...
    def create_entry(parent, label_text, row, column, var=None, validate_func=None): # 修改點: 添加 validate_func 參數
        ...
        if validate_func: # 修改點: 只有在 validate_func 存在的情況下才設定 validate 屬性
    ...
    warehouse_combobox = create_combobox(scrap_window, "選擇倉庫:", 0, 0, warehouse_var, ["倉庫1", "倉庫2"])

    # Item name selection
    name_var = tk.StringVar()
    name_combobox = create_combobox(scrap_window, "物品名稱:", 1, 0, name_var)
    ...
    edit_to_list_button = ttk.Button(button_frame, text="修改清單項目", command=edit_scrap_item_list)
    edit_to_list_button.pack(side="right", padx=5)

    delete_to_list_button = ttk.Button(button_frame, text="刪除清單項目", command=delete_scrap_item_list)
    ...
    def scrap_item_action():
        if messagebox.askyesno("確認", "確定要報廢嗎？"):
            ...
            if scrap_items_list:
                ...
                for (name, expiry_date, warehouse_item), quantity in total_scrap_quantity.items():
                    ...
                    if item_inventory and item_inventory.quantity < quantity:
                        all_success = False
                        messagebox.showerror("錯誤", f"商品 {name}，有效日期 {expiry_date.strftime('%Y-%m-%d')} 庫存不足，庫存{item_inventory.quantity}，報廢{quantity}。")
                        break
            ...
            if all_success:
                ...
                messagebox.showerror("錯誤", "報廢失敗，請檢查輸入。")
