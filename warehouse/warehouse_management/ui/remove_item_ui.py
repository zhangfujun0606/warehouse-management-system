...
def remove_item_window(window, warehouse1, warehouse2, update_inventory, save_data, history_log, logged_in_user):
    ...
    warehouse_label = ttk.Label(remove_window, text="選擇倉庫:")
    warehouse_label.grid(row=0, column=0, padx=5, pady=5)
    warehouse_var = tk.StringVar(value="倉庫1")
    warehouse_combobox = ttk.Combobox(remove_window, textvariable=warehouse_var, values=["倉庫1", "倉庫2"])
    warehouse_combobox.grid(row=0, column=1, padx=5, pady=5)

    name_label = ttk.Label(remove_window, text="物品名稱:")
    name_label.grid(row=1, column=0, padx=5, pady=5)
    name_var = tk.StringVar()
    name_combobox = ttk.Combobox(remove_window, textvariable=name_var)
    name_combobox.grid(row=1, column=1, padx=5, pady=5)

    expiry_date_label = ttk.Label(remove_window, text="有效日期:")
    expiry_date_label.grid(row=2, column=0, padx=5, pady=5)
    expiry_date_var = tk.StringVar()
    expiry_date_combobox = ttk.Combobox(remove_window, textvariable=expiry_date_var)
    ...
    quantity_label = ttk.Label(remove_window, text="數量:")
    ...
    note_label = ttk.Label(remove_window, text="備註:")
    note_label.grid(row=4, column=0, padx=5, pady=5)
    note_entry = ttk.Entry(remove_window)
    ...
    def add_remove_item_to_list():
        ...
        remove_items_list.append((name,expiry_date,quantity, note, warehouse))
    ...
    add_to_list_button = ttk.Button(button_frame, text="新增", command=add_remove_item_to_list)
    add_to_list_button.pack(side="right", padx=5)

    edit_to_list_button = ttk.Button(button_frame, text="修改", command=edit_remove_item_list)
    edit_to_list_button.pack(side="right", padx=5)

    delete_to_list_button = ttk.Button(button_frame, text="刪除", command=delete_remove_item_list)
    ...
    def remove_item_action():
        if messagebox.askyesno("確認", "確定要出貨嗎？"):
            ...
            if remove_items_list:
                if warehouse_name == "倉庫1":
                    for name,expiry_date, quantity,note_item, warehouse_item in remove_items_list:
